...
from src.utils import accuracy, ece, ModelWithTemperature
...
def test_temperature(
    ...
    **kwargs,
    ...
    results_path = root/"results"/"uncertainity"/"temperature"/\
        config_data/config_encoder/config_training
    configs_path = root/"configs"
    
    model_dir = results_path/"models"
    vanilla_dir = root/"results"/"uncertainity"/"vanilla"/\
        config_data/config_encoder/config_training/"models"

    scores_dir = results_path/"scores"

    Path(root/"logs"/"uncertainity").mkdir(parents=True, exist_ok=True)
    logs_path = "{}/logs/uncertainity/test_temperature_{}_{}_{}.log".format(
    ...
    load_epoch_classifier(cfg_training['scheduler']['epochs'], device, vanilla_dir, classifier)
    classifier_temp = ModelWithTemperature(classifier).to(device)
    classifier_temp.set_temperature(test_loader, device)
    logger.info(classifier_temp.temperature)
